...
class uber_network:
    ...
    def handle_client_arrival(self, event: Event, client: uber_client):
        ...
        if client.behavour_type == "Premium":
        ...
        if client.status == "Waiting":
            ...
            if client.behavour_type == "Patient":
                # Patient clients wait 30% longer
                cancellation_time *= 1.3
            elif client.behavour_type == "Premium":
    ...
    def handle_driver_arrival(self, event: Event, driver: uber_driver):
        ...
        if driver.behavour_type == "UberLux":
            shift_duration *= 0.8  # Luxury drivers tend to have shorter shifts
        elif driver.behavour_type == "UberGreen":
        ...
        queue_sizes_before = self.get_queue_sizes()
        self._try_matching_queues()
        queue_sizes_after = self.get_queue_sizes()
        
        # Check if any matches were made
        matches_made = (
            queue_sizes_before['main_queue']['size'] - queue_sizes_after['main_queue']['size'] +
            queue_sizes_before['secondary_queue']['size'] - queue_sizes_after['secondary_queue']['size']
        )
        ...
        self.stats['driver_type_distribution'][driver.behavour_type] += 1
    ...
    def handle_ride_start(self, event: Event, client: uber_client, driver: uber_driver):
        ...
        if driver.behavour_type == "UberX":
            base_speed = 1.0
        elif driver.behavour_type == "UberGreen":
            base_speed = 0.9  # Slightly slower
        elif driver.behavour_type == "UberLux":
        ...
        - Driver Type: {driver.behavour_type}
        """)
    ...
    def handle_ride_end(self, event: Event, client: uber_client, driver: uber_driver):
        ...
        ride_metrics = {
            ...
            'cost': final_cost,
            'client_type': client.behavour_type,
            'driver_type': driver.behavour_type
        }
        ...
        Ride End Details:
        - Time: {self.current_time}
        - Client ID: {client.client_id} ({client.behavour_type})
        - Driver ID: {driver.driver_id} ({driver.behavour_type})
    ...
    def handle_client_cancellation(self, event: Event, client: uber_client):
        ...
        - Time: {self.current_time}
        - Client ID: {client.client_id}
        - Client Type: {client.behavour_type}
    ...
    def _calculate_match_score(self, client: uber_client, driver: uber_driver) -> float:
        ...
        if client.behavour_type == "Premium":
            ...
            elif driver.behavour_type == "UberX":
        ...
        - Client {client.client_id} ({client.behavour_type}) to Driver {driver.driver_id} ({driver.behavour_type})
        - Distance: {distance}
    ...
    def _try_matching_client(self, client: uber_client):
        ...
        if best_score >= base_threshold:
            ...
            if client.behavour_type == "Premium" or best_score >= self.secondary_queue_threshold:
                self.main_queue.put(client)
            ...
            self.unmatched_scores.append({
                ...
                'queue': 'main' if client.behavour_type == "Premium" or 
    ...
    def _try_matching_queue(self, queue_obj: queue.Queue, threshold: float, is_main_queue: bool) -> int:
        ...
        while not queue_obj.empty() and self.available_drivers:
            ...
            if is_main_queue and client.behavour_type == "Premium":
    ...
    def _make_match(self, client: uber_client, driver: uber_driver):
        ...
        Match Created:
        - Time: {self.current_time}
        - Client: {client.client_id} ({client.behavour_type})
        - Driver: {driver.driver_id} ({driver.behavour_type})
    ...
    def get_queue_sizes(self):
        """
        Get detailed information about the current state of both queues.
        
        Returns:
            dict: Dictionary containing:
                - queue sizes
                - client types in each queue
                - average wait times
                - longest waiting client info
        """
        ...
        def analyze_queue(queue_obj, queue_type):
            ...
            while not queue_obj.empty():
                ...
                # Update statistics
                queue_info[queue_type]['client_types'][client.behavour_type] += 1
                total_wait_time += wait_time
                clients_processed += 1
                
                # Check for longest wait
                if wait_time > queue_info[queue_type]['longest_wait']:
                    queue_info[queue_type]['longest_wait'] = wait_time
                    queue_info[queue_type]['longest_waiting_client'] = {
                        'client_id': client.client_id,
            ...
            if clients_processed > 0:
                queue_info[queue_type]['avg_wait_time'] = total_wait_time / clients_processed
