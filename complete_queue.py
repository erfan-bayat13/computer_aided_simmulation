...
class Client:
    ...
    def arrival(time, FES, waiting_queues, arrival_rate, service_rate, servers, metrics, is_preemptive=False):
        ...
        if is_preemptive:
            preempted = False
            # Check if there are any servers that can be preempted
            if (new_client.priority == 3) and any(server is not None and server.priority < 3 for server in servers):
                for i, server in enumerate(servers):
                    if server is not None and server.priority < 3:
                        preempted_client = server
                        current_time = time
                        time_served = current_time - preempted_client.last_service_start
                        preempted_client.remaining_service_time -= time_served
                        preempted_client.last_service_start = None
                        servers[i] = new_client
                        new_client.last_service_start = current_time
                        preempted = True
                        #print(f"Preempted {preempted_client.client_type} client from server {i} at {time:.2f}.")
                        metrics['priority_metrics'][priority]['arrived'] += 1
                        # Remove the departure event for the preempted client
                        FES = [event for event in FES if not (event[1] == "departure" and event[2] == i)]
                        heapq.heapify(FES)  # Re-heapify the event set after modification
                        
                        # Schedule departure for the new client
                        departure_time = time + new_client.remaining_service_time
                        heapq.heappush(FES, (departure_time, "departure", i))
                        
                        # Add the preempted client back to the queue
                        waiting_queues[preempted_client.priority].put(preempted_client)
                        
                        metrics['preemptions'] += 1
                        break
                if preempted:
                    return False # Indicate that the client was not rejected
        ...
        for i, server in enumerate(servers):
            if server is None:
                ...
                if highest_priority_client:
                    ...
                    highest_priority_client.last_service_start = time
                    service_time = random.expovariate(service_rate)
        ...
        if isinstance(waiting_queues, dict):
            for priority in sorted(waiting_queues.keys(), reverse=True):
                if not waiting_queues[priority].empty():
                    ...
                    departure_time = time + next_client.remaining_service_time
...
class MultiServerQueueSimulator:
    def __init__(self, num_servers, queue_capacity, arrival_rate, service_rate, simulation_time, queue_type="FIFO", is_preemptive=False):
        ...
        elif queue_type == "Priority" or is_preemptive:
    ...
    def print_summary(self):
        ...
        print("\nSimulation Summary:")
        print(f"Queue Type: {'Preemptive' if self.is_preemptive else 'Non-preemptive Priority'} Queue")
...
simulator = MultiServerQueueSimulator(num_servers, queue_capacity, arrival_rate, service_rate, simulation_time, queue_type="priority", is_preemptive=True)
simulator.run()